...
def ___RecodeSampah__():
	try:
		...
		try:
			...
			______DICKYXD______(f'{P} [{M}!{P}] Token Expire He Jani ')
			______LuWibu______() 
...
def __cekakun__():
    ...
    ______DICKYXD______(f" {P}[{H}2{P}] Cek Results CP ")
    ...
    if anjg == '':
        ...
        ______DICKYXD______( ' [!] Select yang benar!!')
        ___SayaRecodeSampah___()
def __upgrade__():
...
def _____Publik_______():
	try:
		...
		______LuWibu______()
	______DICKYXD______(P+'\n ['+h+'•'+P+'] Enter Only Public Uid ')
	pil = ______DickyGans______(P+' ['+h+'?'+P+'] Enter ID Target : ')
	try:
	...
	try:
		...
		______LuWibu______()
	______DICKYXD______(f" \n {P}[{H}?{P}] do you want to use manual password? (choase t) [Y/t] ")
	ask=______DickyGans______(" %s[%s?%s] %sSelect :%s "%(p,H,p,p,p))
	if ask in[""]:
		______LuWibu______(f" {P}[{H}!{P}] Selectan Tidak Ada")
	elif ask in["t"]:
		_____LANGSUNG_____()
	elif ask in["y"]:
		____MANUAL_____()
...
def _____LANGSUNG_____():
	______DICKYXD______("")
	______DICKYXD______(' %s[%s+%s] %sResults %sOK %ssaved to -> %sok.txt'%(p,p,p,p,p,p,H))
	______DICKYXD______(' %s[%s+%s] %sResults %sCP %ssaved to -> %scp.txt'%(p,p,p,p,p,p,K))
	______DICKYxXD______(" %s[%s!%s] %sIf There %sIs No %sResults Use Airplane Mode -> %s5 Seconds\n"%(p,M,p,p,p,p,p))
...
def ___SayaRecodeSampah___():
	...
	______DICKYXD______(f" \n {P}[{H}1{P}] Public ID Clonning \n {P}[{H}2{P}] Check Results OK/CP \n {P}[{H}3{P}] Setting UserAgent \n {P}[{H}4{P}] Upgrade {H}Pro \n {P}[{H}0{P}] Get Out (Logout)")
	ask = ______DickyGans______(f" {P}[{U}?{P}] Select : ")
	if ask =="":______LuWibu______(f" {P}[{H}!{P}] Selectan Tidak Ada")
	...
	else:______LuWibu______(f" {P}[{H}!{P}] Selectan Tidak Ada")
